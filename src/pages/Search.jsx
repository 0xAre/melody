    ...
    <div className="p-6">
      ...
      <div className="relative mb-8">
        ...
        <input
          ...
          className="w-full max-w-xl bg-surface-3 text-white placeholder-gray-500 rounded-full py-3 pl-12 pr-4 outline-none border border-transparent focus:border-primary transition-colors text-sm"
        />
        {query && (
            ...
            {MOODS.map((mood) => {
              ...
              return (
                <button
                  ...
                  className={`bg-gradient-to-br ${meta.bg} rounded-xl p-4 text-left hover:scale-105 transition-transform h-24 relative overflow-hidden`}
                >
                  <p className="font-bold text-white">{mood}</p>
                  <p className="text-white/70 text-xs">{count} lagu</p>
                  <span className="absolute bottom-2 right-3 text-3xl opacity-40">{meta.emoji}</span>
                </button>
              )
            })}
          </div>
